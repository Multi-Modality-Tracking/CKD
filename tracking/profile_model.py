...
def parse_args():
    ...
    # for train
    parser.add_argument('--script', type=str, default='ckd', choices=['ostrack', 'ckd'],
                        help='training script name')
    parser.add_argument('--config', type=str, default='ckd_4b_dropmae_ckd_tf_cc_mask.25', help='yaml configure file name')
...
if __name__ == "__main__":
    ...
    elif args.script == "ckd":
        
        model_module = importlib.import_module('lib.models')
        model_constructor = model_module.build_ostrack_ckd
        model = model_constructor(cfg, training=False)
        model.eval()
        # get the template and search
        template = torch.randn(2, bs, 3, z_sz, z_sz)
        search = torch.randn(2, bs, 3, x_sz, x_sz)
        # transfer to device
        model = model.to(device)
        template = template.to(device)
        search = search.to(device)

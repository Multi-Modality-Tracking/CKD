...
class STARKProcessing_RGBT(BaseProcessing):
    ...
    def __call__(self, data: TensorDict):
        ...
        for s in ['template', 'search']:
            ...
            crops, boxes, att_mask, mask_crops = prutils.jittered_center_crop_rgbt(data[s + '_images'], jittered_anno,
            ...
            # 这里因为我们的数据读取方式的差异，需要把joint改成True【如果你读取两模态图片是通过6个通道读取的，则不需要】
            data[s + '_images'], data[s + '_anno'], data[s + '_att'], data[s + '_masks'] = self.transform[s](
                image=crops, bbox=boxes, att=att_mask, mask=mask_crops, joint=True) # joint表示两个模态的数据是否采用同样的随机数来做数据增强

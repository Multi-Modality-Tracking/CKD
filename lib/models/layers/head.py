...
class CenterPredictor(nn.Module, ):
    ...
    def get_score_map(self, x):
        ...
        def _sigmoid(x):
            y = torch.clamp(x.sigmoid_(), min=1e-4, max=1 - 1e-4)   # 去掉特别小和特别大的值
            return y
...
def build_box_head(cfg, hidden_dim):
    ...
    if cfg.MODEL.HEAD.TYPE == "MLP":
        ...
        if cfg.MODEL.HEAD.TYPE == "CORNER":
            ...
            raise ValueError()
        return corner_head
    elif cfg.MODEL.HEAD.TYPE == "CENTER":  ### ostrack selecte this head to predict
        in_channel = hidden_dim ## for RGBT feature cat
